...
module.exports.createSubcategory = asyncHandler(async (req, res) => {
	...
	if (!category_id)
		return res
			.status(400)
			.send({ success: false, message: "No Category Id Provided." });
...
module.exports.getSubCategoryByCatId = asyncHandler(async (req, res) => {
	let id = req.params.id,
		data;
	data = await crudOPs.getData("subcategories", null, { categor });
	return res.send({ success: true, data });
});
